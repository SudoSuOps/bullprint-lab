# Handoff: BULLPRINT LAB — full site + product system

_Last synced 2026-08-15 (board-review build). Every design file in `design/` is current as of this date._

Head extras already in the reference: `<title>`, meta description, `scroll-margin-top: 112px` on sections (sticky-header offset), external links `target="_blank" rel="noopener"`. Carry all four into the build.
...
A complete, working design for the BullPrint Lab marketing + commerce site: 14 sections, three interactive instruments, two lead-capture forms and one order build sheet. Build it in the repo as a real app.
...
|---|---|---|---|
| — | `hero` | Hero — **WE PRINT.** + genesis-hash line, dual CTA, faded line-cut bull backdrop, seal | — |
...
| 07 | `brahma` | BrAhMa — the AI in the lab | — |
| 08 | `bull` | BEST IN BULL™ — genesis-hash seal | — |
| 09 | `fabric` | Bull Fabric — BrAhMa weave, hash strip, headbands | — |
| 10 | `drops` | Drops grid — 4 cards, 5 possible statuses | hover states |
| 11 | `journal` | THE BULLPRINT — featured essay + 3 drafts | hover states |
| 12 | `order` | Tell us about your shoes — **build sheet + checkout** | 6 fields + validation + confirm |
| 13 | `about` | About + stats | — |
| 14 | `contact` | Contact us — tiles + message form | form + validation + confirm |
| — | — | Footer | — |
...
design/BullPrint Store.dc.html      store: 3 footwear drops, materials standard, heel-cup plate, capacity
design/BullSlides CAD.dc.html       controlled drawing BPL-SL-013-B — 5 sheets, size 13
design/BullSlides Two-Layer.dc.html 3-part captured-band construction + ₿ULLISH rail lockup
design/BullPrint Monogram.dc.html   band monograms, 7 fabric prints, Bull Bands tiles
design/BullPrint Badges.dc.html     10 drop-in badges, 4 sculpted band profiles, engraving type
design/BullPrintLab Platform.dc.html AI-CAD platform UI — phase 1 interface, 4 screens
design/BullPrint Lab.dc.html        design board: early exploration, mark development
design/support.js                   runtime for the reference files only. do not ship.
```
